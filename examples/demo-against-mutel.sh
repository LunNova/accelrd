...
#
# Run accelrd once against a localhost mutel and print the
...
# ACCELRD_OTLP_ENDPOINT). Pass extra args through, e.g.:
#   ./examples/demo-against-mutel.sh --rack rack-A --block block-1
set -euo pipefail

cd "$(dirname "$0")/.."

: "${ACCELRD_OTLP_ENDPOINT:=http://127.0.0.1:4318}"
: "${RUST_LOG:=info,opentelemetry=warn,reqwest=error}"
export ACCELRD_OTLP_ENDPOINT RUST_LOG
