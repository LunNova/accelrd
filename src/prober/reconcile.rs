...
//! One reconcile pass: list nodes → group by rack → pick LRU pair →
//! orchestrate the two-pod probe → patch annotations. Designed to be
//! called on a timer; idempotent against partial state because each
//! probe is its own pair-id-tagged Pod set, and stale tags get cleaned
//! up at the start of the next pass.
...
use crate::prober::annotations::{Verdict, patch_pair};
use crate::prober::pair::{NodeView, eligible, group_by_rack, pick_pair};
...
pub async fn run_once(client: &Client, args: &Resolved) {
	...
	for (rack, members) in racks {
		let Some(pair) = pick_pair(&members, &cutoff) else {
			continue;
		};
